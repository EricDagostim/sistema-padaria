<?php
    ...
    if(!empty($_GET['id']))
    {
      ...
      $id = $_GET['id'];
      $select = mysqli_query($conexao, "SELECT * FROM produtos WHERE id=$id");       
    
      
      if ($select->num_rows > 0){
       
        while($prod = mysqli_fetch_assoc($select)){
            $nome = $prod['nome'];
            $vl_unitario = $prod['vl_unitario'];
            $ps_unitario = $prod['ps_unitario'];  
    ...
    if(isset($_POST['editar'])){
        ...
        $sqlAtualiza = mysqli_query($conexao, "UPDATE produtos SET nome='$nome', vl_unitario='$vl_unitario', ps_unitario='$ps_unitario', quantidade='$quantidade'
        WHERE id='$id'");
            ...
            <div class="row">
                <div class="col-6 offset-3">
                    ...
                    <div class="input-group mb-3">
                          ...
                          <input type="text" name="nome" class="form-control" placeholder="Nome produto *" aria-label="Nome" aria-describedby="basic-addon1" required value=" <?= $nome ?> ">
                        </div>
